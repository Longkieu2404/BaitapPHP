...
<?php
	...
	$ketqua="";
	if (isset($_POST['Chon_pheptinh'])&&$mauso_1!=0 && $mauso_2!=0) 
	{
		...
		{	case "cộng": 
						...
						$ketqua=$ps_1->get_tuso()."/".$ps_1->get_mauso()."+".$ps_2->get_tuso()."/".$ps_2->get_mauso()."=";
                        if($ps->get_mauso()==$ps->get_tuso()) $ketqua.=1;
                        else if($ps->get_tuso()==0) $ketqua.=0;
                                else if($ps->get_mauso()==1) $ketqua.=$ps->get_tuso();
                                        else $ketqua.=$ps->get_tuso()."/".$ps->get_mauso();
						break;
			case "trừ":
						$ps=new PHAN_SO();
            ...
            case "chia":
                        ...
                        {
                            $ps=new PHAN_SO();
                            $ps=$ps_1->nhan($ps_2->mauso,$ps_2->tuso);
                            $ketqua=$ps_1->get_tuso()."/".$ps_1->get_mauso()."/".$ps_2->get_tuso()."/".$ps_2->get_mauso()."=";
                            if($ps->get_mauso()==$ps->get_tuso()) $ketqua.=1;
                            else if($ps->get_tuso()==0) $ketqua.=0;
                                    else if($ps->get_mauso()==1) $ketqua.=$ps->get_tuso();
                        ...
                        {
                            echo "<font color = 'red'>Tử số 2 phải khác 0</font>";
                            $ketqua="";
                        }
								
		}
        if($ketqua!="") $ketqua="Phép ".$pheptinh." là : ". $ketqua;
        echo $ketqua;
	}
    else if($mauso_2==0 || $mauso_1==0) echo "<font color = 'red'>Mẫu số phải khác 0</font>";
